package program;
/**
 * File name: GroupEditor.java
 * ====================================================
 * This class initializes the window that  
 * allows you to edit a chosen product group.
 */
import javax.swing.*;
import java.awt.*;
...
public class GroupEditor {
	...
	private String groupName, groupDesc, help; // Variables for the name of the group and for the description. And the helping variable.
	...
	private void initialize() {
		...
		frmGroupAdder.setTitle("Edit product group");
		...
		nameTextField.setText(
				GroupsWindow.getGroupsTableModel().getValueAt(GroupsWindow.getGroupsTable().getSelectedRow(), 0));
		help = nameTextField.getText();
		...
		JLabel labelGroupDescription = new JLabel("Group description: ");
		panel_1.add(labelGroupDescription);

		descTextField = new JTextField();
		descTextField.setText(
				GroupsWindow.getGroupsTableModel().getValueAt(GroupsWindow.getGroupsTable().getSelectedRow(), 1));
		 ...
		 * will edit chosen product group. Changes will be added to the table and to
		...
		btnConfirm.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (!groupName.equals("") && groupName.trim().length() > 0) {
					if (groupName.equals(help)) {
						GroupsWindow.getGroupsTableModel()
								.editProductGroup(GroupsWindow.getGroupsTable().getSelectedRow(), groupName, groupDesc);
						frmGroupAdder.dispose();
						GroupsWindow.getFrmGroupsWindow().setEnabled(true);
					} else {
						if (!GroupsWindow.getGroupsTableModel().checkName(groupName)) {
							GroupsWindow.getGroupsTableModel().editProductGroup(
									GroupsWindow.getGroupsTable().getSelectedRow(), groupName, groupDesc);
							frmGroupAdder.dispose();
							GroupsWindow.getFrmGroupsWindow().setEnabled(true);
